/**
 * NAPI prayer times unit tests — mirrors tests/wasm/prayers.test.ts
 * but uses the native napi-rs implementation.
 */
import { test, expect, describe } from "bun:test";
import { computePrayerTimes } from "../../src/napi/index.ts";
...
describe("NAPI: Chittagong — MWL Hanafi, Feb 25 2026", () => {
  ...
  test("Asr (Hanafi) is 16:17", () => {
    expect(
      Math.abs(diffMin("16:17", getHHMM(result, "asr", TZ))),
    ).toBeLessThanOrEqual(1);
  });

  test("Maghrib is 17:55", () => {
    expect(
  ...
  test("Firstthird is 22:02 (±1 min)", () => {
    expect(
      Math.abs(diffMin("22:02", getHHMM(result, "firstThird", TZ))),
    ).toBeLessThanOrEqual(1);
  });

  test("Lastthird is 02:10 (±1 min)", () => {
    expect(
...
describe("NAPI: Chittagong — Karachi Hanafi, Feb 25 2026", () => {
...
describe("NAPI: London — MWL Hanafi, Summer Solstice Jun 21 2026", () => {
  ...
  test("Fajr is 02:31 (±2 min, NAPI direct approach)", () => {
    expect(
      Math.abs(diffMin("02:31", getHHMM(result, "fajr", TZ))),
    ).toBeLessThanOrEqual(2);
  });

  test("Sunrise is 04:43", () => {
    expect(
...
describe("NAPI: New York — ISNA Hanafi, Dec 15 2026", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Mecca — Umm al-Qura Hanafi, Feb 25 2026", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Cairo — MWL Shafi, Jun 21 2022", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Sydney — Karachi Hanafi, Dec 21 2026", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Jakarta — Singapore Shafi, Dec 25 2023", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Polar region — undefined sunrise/sunset", () => {
  const config: PrayerTimeConfig = {
...
describe("NAPI: Sunset vs Maghrib with adjustment", () => {
...
describe("NAPI: Hanafi vs Standard Asr difference", () => {
...
describe("NAPI: Dhuhr adjustment", () => {
...
describe("NAPI: elevation adjusts sunrise/sunset", () => {
