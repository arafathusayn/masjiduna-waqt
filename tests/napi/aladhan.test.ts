/**
 * NAPI E2E test — mirrors tests/wasm/aladhan.test.ts
 * Compares NAPI prayer time output against Aladhan API fixtures.
 */
...
// NAPI direct (no-refine) approach may have slightly larger error than JS
const TOLERANCE_MINUTES = 2;
const LOG_PATH = "test-results/napi-e2e.log";
...
describe("NAPI E2E: Aladhan fixture comparison", () => {
  beforeAll(async () => {
    ...
    log(`Tolerance: ${TOLERANCE_MINUTES} minute(s) (NAPI direct approach)`);
  });

  afterAll(async () => {
    log("");
    log("=".repeat(72));
    log("NAPI E2E SUMMARY");
    log("=".repeat(72));
