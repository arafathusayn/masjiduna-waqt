/**
 * WASM context parity test — verifies createPrayerContext().compute()
 * returns identical results to computePrayerTimes() across 20 locations × 365 days.
 *
 * Note: WASM adapter always delegates to the same code path, so this primarily
 * validates that the context wrapper correctly forwards config and produces
 * consistent output.
 */
...
describe("WASM: context API parity with computePrayerTimes", () => {
